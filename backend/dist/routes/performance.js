...
const runPrediction = async (data) => {
    return new Promise((resolve, reject) => {
        const pythonScript = path_1.default.join(__dirname, '../../ml/predict_json.py');
        const pythonProcess = (0, child_process_1.spawn)('python', [pythonScript], { timeout: 10000 });
        let result = '';
        let errorOutput = '';
...
router.post('/', auth_1.authenticate, async (req, res) => {
    ...
    try {
        const difficultiesPath = path_1.default.join(__dirname, '../../ml/subject_difficulties.json');
        const difficulties = JSON.parse(fs_1.default.readFileSync(difficultiesPath, 'utf8'));
    ...
    try {
        // Get active semester
        const semRes = await (0, db_1.query)('SELECT id FROM semesters WHERE is_active = true LIMIT 1');
...
router.post('/:id/share', auth_1.authenticate, async (req, res) => {
    ...
    `, [id, user.id]);
        if (updateRes.rowCount === 0)
            return res.status(404).json({ error: 'Performance record not found or unauthorized' });
...
router.get('/', auth_1.authenticate, async (req, res) => {
    ...
    try {
      ...
      `;
            const dbRes = await (0, db_1.query)(q, params);
...
router.post('/predict-marks', auth_1.authenticate, async (req, res) => {
    const { user } = req;
    if (user.role !== 'STUDENT')
        return res.status(403).json({ error: 'Unauthorized' });
    ...
    try {
        const difficultiesPath = path_1.default.join(__dirname, '../../ml/subject_difficulties.json');
        if (fs_1.default.existsSync(difficultiesPath)) {
            const difficulties = JSON.parse(fs_1.default.readFileSync(difficultiesPath, 'utf8'));
            if (difficulties[subject]) {
                subject_difficulty = difficulties[subject];
            }
    ...
    try {
        const cgpaRes = await (0, db_1.query)('SELECT cgpa FROM cgpa_records WHERE student_id = $1', [user.id]);
