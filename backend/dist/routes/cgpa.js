...
router.post('/upload', auth_1.authenticate, async (req, res) => {
    ...
    try {
        ...
        for (const record of records) {
            ...
            const studentRes = await (0, db_1.query)('SELECT id FROM users WHERE email = $1 AND role = $2', [student_email, 'STUDENT']);
...
router.get('/me', auth_1.authenticate, async (req, res) => {
    if (req.user.role !== 'STUDENT')
        return res.status(403).json({ error: 'Unauthorized' });
    try {
        const dbRes = await (0, db_1.query)('SELECT cgpa FROM cgpa_records WHERE student_id = $1', [req.user.id]);
        if (dbRes.rowCount === 0)
            return res.json({ cgpa: null });
