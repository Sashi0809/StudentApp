  ...
  try {
    if (is_official && user.role === 'HOD' && user.department_id) {
      const dbRes = await query(
        'INSERT INTO events (department_id, target_year, event_date, title, description) VALUES ($1, $2, $3, $4, $5) RETURNING *',
        [user.department_id, target_year || 0, event_date, title, description]
      );
      return res.status(201).json(dbRes.rows[0]);
    } else {
      const dbRes = await query(
        'INSERT INTO events (user_id, event_date, title, description) VALUES ($1, $2, $3, $4) RETURNING *',
        [user.id, event_date, title, description]
      );
      return res.status(201).json(dbRes.rows[0]);
    }
  } catch (error) {
...
router.delete('/:id', authenticate, async (req, res) => {
  ...
  try {
    ...
    if (user.role === 'HOD' && user.department_id) {
      dbRes = await query(
        'DELETE FROM events WHERE id = $1 AND (user_id = $2 OR department_id = $3) RETURNING id',
