...
// POST material
router.post('/:id/materials', authenticate, upload.single('file'), async (req, res) => {
  const user = req.user;
  const { id } = req.params;
...
// POST assignment
router.post('/:id/assignments', authenticate, upload.single('file'), async (req, res) => {
  const user = req.user;
  const { id } = req.params;
...
router.get('/:id/assignments/:assignmentId/submissions', authenticate, async (req, res) => {
  const user = req.user;
...
// POST submission
router.post('/:id/assignments/:assignmentId/submit', authenticate, upload.single('file'), async (req, res) => {
  const user = req.user;
  const { id, assignmentId } = req.params;
  if (!user) return res.status(401).json({ error: 'Unauthorized' });
