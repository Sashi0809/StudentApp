...
export default function HodDashboard() {
 ...
 activeTab === 'overview' ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-gray-500 hover:text-gray-800'
 }`}
 >
 ...
 className={`px-6 py-3 font-medium flex items-center gap-2 border-b-2 transition-colors ${
 activeTab === 'semesters' ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-gray-500 hover:text-gray-800'
 }`}
 >
 ...
 className={`px-6 py-3 font-medium flex items-center gap-2 border-b-2 transition-colors ${
 activeTab === 'cgpa' ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-gray-500 hover:text-gray-800'
 }`}
 >
 ...
 className={`px-6 py-3 font-medium flex items-center gap-2 border-b-2 transition-colors ${
 activeTab === 'complaints' ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-gray-500 hover:text-gray-800'
 }`}
 >
 ...
 <div key={teacher.id} className="bg-gray-50 border border-yellow-500/20 rounded-lg p-4 flex items-center justify-between gap-4">
 ...
 <button onClick={() => handleStatusChange(teacher.id, 'APPROVED')} className="p-1.5 bg-emerald-500/20 hover:bg-emerald-500/40 text-emerald-600 rounded transition-colors" title="Approve">
 <Check size={18} />
 </button>
 <button onClick={() => handleStatusChange(teacher.id, 'REJECTED')} className="p-1.5 bg-red-500/20 hover:bg-red-500/40 text-red-600 rounded transition-colors" title="Reject">
 <X size={18} />
 </button>
 </div>
 </div>
 ...
 <UploadCloud className="text-emerald-600" /> Upload Department Timetable
 </h2>
 
 <form onSubmit={handleUpload} className="max-w-2xl">
 <div className="border-2 border-dashed border-gray-300 hover:border-emerald-500/50 transition-colors rounded-xl p-8 text-center bg-black/20">
 <input 
 type="file" 
 id="timetable-upload"
 accept=".pdf,image/*"
 ...
 className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 rounded-lg text-gray-900 font-medium disabled:opacity-50 transition-colors shadow-lg"
 >
 ...
 <div key={teacher.id} className="bg-white border border-gray-200 rounded-lg p-4 flex items-center justify-between gap-4 hover:border-emerald-500/30 transition-colors">
 <div className="flex items-center gap-4">
