...
export default function SemesterManager() {
  ...
  if (loading) return <div className="text-gray-400">Loading semesters...</div>;

  return (
    <div className="space-y-8">
      
      {/* Active Semester Status */}
      <div className={`p-6 rounded-xl border ${activeSemester ? 'bg-blue-900/20 border-blue-500/30' : 'bg-black/20 border-white/10'}`}>
        <h3 className="text-xl font-semibold text-white mb-4 flex items-center gap-2">
          <Calendar className={activeSemester ? "text-blue-400" : "text-gray-400"} /> 
        ...
        {activeSemester ? (
          ...
          <p className="text-gray-400">There is currently no active semester. Start a new one below.</p>
        )}
      </div>

      {/* Start New Semester */}
      <div className="bg-white/5 border border-white/10 rounded-xl p-6">
        <h3 className="text-lg font-semibold text-white mb-4">Start New Semester</h3>
        
        <form onSubmit={handleCreate} className="max-w-2xl space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm text-gray-300 mb-1">Semester Name</label>
              <input
                required
                type="text"
                value={name}
                ...
                className="w-full bg-black/30 border border-white/20 rounded-lg px-3 py-2 text-white focus:border-blue-500"
              />
            </div>
            <div>
      ...
      {/* Messages */}
      {message.text && (
        <div className={`p-4 rounded-lg border flex items-center gap-2 ${message.type === 'error' ? 'bg-red-500/10 border-red-500/30 text-red-400' : 'bg-green-500/10 border-green-500/30 text-green-400'}`}>
      ...
      {/* History */}
      <div className="bg-white/5 border border-white/10 rounded-xl p-6">
        <h3 className="text-lg font-semibold text-white mb-4">Semester History</h3>
        {semesters.length === 0 ? (
          <p className="text-gray-400">No semesters recorded.</p>
        ) : (
            ...
            <tbody className="divide-y divide-white/5">
              {semesters.map(s => (
                <tr key={s.id}>
                  ...
                  <td className="py-3 text-gray-400 text-sm">{new Date(s.start_date).toLocaleDateString()}</td>
                  <td className="py-3 text-gray-400 text-sm">{new Date(s.end_date).toLocaleDateString()}</td>
                  <td className="py-3">
                    {s.is_active ? (
                      <span className="px-2 py-1 bg-blue-500/20 text-blue-400 text-xs rounded-full font-medium">Active</span>
                    ) : (
                      <span className="px-2 py-1 bg-gray-500/20 text-gray-400 text-xs rounded-full font-medium">Completed</span>
                    )}
                  </td>
                </tr>
              ))}
