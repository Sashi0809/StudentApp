...
export default function ComplaintsInbox() {
  ...
  if (loading) return <div className="text-gray-400">Loading complaints...</div>;

  return (
    <div className="bg-white/5 border border-white/10 rounded-xl p-6">
      <h3 className="text-xl font-semibold text-white mb-6 flex items-center gap-2">
        <MessageSquare className="text-red-400" /> Student Complaints
      </h3>

      {complaints.length === 0 ? (
        <div className="text-center py-8 text-gray-400 bg-black/20 rounded-lg border border-white/5">
              ...
              <div className="flex justify-between items-start gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-gray-400">({c.student_email})</span>
                  </div>
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-400">
                    ...
                    <span className={`px-2 py-0.5 rounded-full font-medium ${c.status === 'RESOLVED' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                      {c.status}
                    </span>
                  </div>
                </div>
